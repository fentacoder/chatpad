...
public class TypeToken<T> {
  ...
  private static boolean isAssignableFrom(Type paramType, ParameterizedType paramParameterizedType, Map<String, Type> paramMap) {
    ...
    for (byte b2 = b1; b2 < i; b2++) {
      if (isAssignableFrom(arrayOfType[b2], paramParameterizedType, new HashMap<>(paramMap)))
        return true; 
    } 
    return isAssignableFrom(clazz.getGenericSuperclass(), paramParameterizedType, new HashMap<>(paramMap));
  }
...
/* Location:              C:\Users\jamar\Documents\android_location\apk_conversion\dex2jar-3.0\classes2-dex2jar.jar!\com\google\gson\reflect\TypeToken.class
