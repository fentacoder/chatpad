...
public final class MultipartBuilder {
  ...
  private static final class MultipartRequestBody extends RequestBody {
    ...
    private long writeOrCountBytes(BufferedSink param1BufferedSink, boolean param1Boolean) throws IOException {
      ...
      int i = this.partHeaders.size();
      long l1 = 0L;
      for (byte b = 0; b < i; b++) {
        Headers headers = this.partHeaders.get(b);
        RequestBody requestBody = this.partBodies.get(b);
        ...
        if (headers != null) {
          ...
          for (byte b1 = 0; b1 < j; b1++)
            buffer2.writeUtf8(headers.name(b1)).write(MultipartBuilder.COLONSPACE).writeUtf8(headers.value(b1)).write(MultipartBuilder.CRLF); 
        } 
        ...
        if (param1Boolean) {
          ...
          ((RequestBody)this.partBodies.get(b)).writeTo((BufferedSink)buffer2);
        } 
        buffer2.write(MultipartBuilder.CRLF);
      } 
      buffer2.write(MultipartBuilder.DASHDASH);
...
/* Location:              C:\Users\jamar\Documents\android_location\apk_conversion\dex2jar-3.0\classes2-dex2jar.jar!\com\squareup\okhttp\MultipartBuilder.class
