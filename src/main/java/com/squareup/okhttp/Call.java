  ...
  Response getResponse(Request paramRequest, boolean paramBoolean) throws IOException {
    ...
    while (!this.canceled) {
      ...
      try {
        ...
        request = this.engine.followUpRequest();
        if (request == null)
          return response; 
        StreamAllocation streamAllocation = this.engine.close();
        if (++b <= 20) {
          if (!this.engine.sameConnection(request.httpUrl())) {
            streamAllocation.release();
            streamAllocation = null;
          } 
          this.engine = new HttpEngine(this.client, request, false, false, paramBoolean, streamAllocation, null, response);
          continue;
        } 
        streamAllocation.release();
        StringBuilder stringBuilder = new StringBuilder();
...
/* Location:              C:\Users\jamar\Documents\android_location\apk_conversion\dex2jar-3.0\classes2-dex2jar.jar!\com\squareup\okhttp\Call.class
