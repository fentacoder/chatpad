...
public final class HttpEngine {
  ...
  private static Address createAddress(OkHttpClient paramOkHttpClient, Request paramRequest) {
    ...
    if (paramRequest.isHttps()) {
      sSLSocketFactory1 = paramOkHttpClient.getSslSocketFactory();
      sSLSocketFactory2 = (SSLSocketFactory)paramOkHttpClient.getHostnameVerifier();
      sSLSocketFactory3 = (SSLSocketFactory)paramOkHttpClient.getCertificatePinner();
    } else {
      sSLSocketFactory1 = null;
      sSLSocketFactory2 = sSLSocketFactory1;
      sSLSocketFactory3 = sSLSocketFactory2;
    } 
    return new Address(paramRequest.httpUrl().host(), paramRequest.httpUrl().port(), paramOkHttpClient.getDns(), paramOkHttpClient.getSocketFactory(), sSLSocketFactory1, (HostnameVerifier)sSLSocketFactory2, (CertificatePinner)sSLSocketFactory3, paramOkHttpClient.getAuthenticator(), paramOkHttpClient.getProxy(), paramOkHttpClient.getProtocols(), paramOkHttpClient.getConnectionSpecs(), paramOkHttpClient.getProxySelector());
  }
  ...
  private Response readNetworkResponse() throws IOException {
    ...
    Response response1 = this.httpStream.readResponseHeaders().request(this.networkRequest).handshake(this.streamAllocation.connection().getHandshake()).header(OkHeaders.SENT_MILLIS, Long.toString(this.sentRequestMillis)).header(OkHeaders.RECEIVED_MILLIS, Long.toString(System.currentTimeMillis())).build();
    Response response2 = response1;
    if (!this.forWebSocket)
      response2 = response1.newBuilder().body(this.httpStream.openResponseBody(response1)).build(); 
  ...
  class NetworkInterceptorChain implements Interceptor.Chain {
    ...
    public Response proceed(Request param1Request) throws IOException {
      ...
      if (this.index > 0) {
        ...
        if (param1Request.httpUrl().host().equals(address.getUriHost()) && param1Request.httpUrl().port() == address.getUriPort()) {
          if (this.calls > 1) {
            ...
            stringBuilder2.append(interceptor1);
          ...
          stringBuilder2.append("network interceptor ");
          stringBuilder2.append(interceptor1);
          stringBuilder2.append(" must retain the same host and port");
          throw new IllegalStateException(stringBuilder2.toString());
        } 
      } 
      if (this.index < HttpEngine.this.client.networkInterceptors().size()) {
        NetworkInterceptorChain networkInterceptorChain = new NetworkInterceptorChain(this.index + 1, (Request)stringBuilder2);
      ...
      stringBuilder1.append(" had non-zero Content-Length: ");
      stringBuilder1.append(response.body().contentLength());
...
/* Location:              C:\Users\jamar\Documents\android_location\apk_conversion\dex2jar-3.0\classes2-dex2jar.jar!\com\squareup\okhttp\internal\http\HttpEngine.class
